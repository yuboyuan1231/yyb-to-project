...
class VideoBank:
    ...
    def build_or_load_sequence_mask(
        ...
        vids = video_ids[: int(max_videos)] if max_videos and max_videos > 0 else video_ids
        suffix = "all" if not max_videos else f"top{int(max_videos)}"
        path = TMP_ROOT / f"C28C_VIDEO_VISUAL_SEQ_MASK_{suffix}_T{int(target_len)}.npy"
        manifest_path = TMP_ROOT / f"C28C_VIDEO_VISUAL_SEQ_MASK_{suffix}_T{int(target_len)}.manifest.json"
        if path.exists() and manifest_path.exists() and not force:
        ...
        manifest = {
            ...
            "video_count": len(vids),
            "target_len": int(target_len),
            "schema_hash": stable_hash({"video_count": len(vids), "target_len": int(target_len), "kind": "visual_sequence_mask"}),

#!/usr/bin/env python3
"""C12-5U topK promotion + calibration repair.

Train-only two-head and calibration repair over C12 native generated M=1000
span pools.
...
OUT = ROOT / "c12_5u_topk_calibration_repair"
MODEL_DIR = ROOT / "c12_models"
SEED = 1285
TRAIN_LIMIT = int(os.environ.get("C12_5U_TRAIN_LIMIT", "12000"))
EVAL_LIMIT = int(os.environ.get("C12_5U_EVAL_LIMIT", "0"))
TRAIN_BATCH = int(os.environ.get("C12_5U_TRAIN_BATCH", "48"))
EVAL_BATCH = int(os.environ.get("C12_5U_EVAL_BATCH", "48"))
POOL_LIMIT = 1000
MAX_SPAN = 64
...
class TwoHeadRanker(nn.Module):
    def __init__(self, in_dim: int, hidden: int = 224, dropout: float = 0.10) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        h = self.trunk(x)
        return self.select_head(h).squeeze(-1), self.quality_head(h).squeeze(-1)
...
def train_two_head_ranker(groups: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    seed_all(SEED)
    idxs = variant_feature_indices("S2_span_context_contrast")
    model = TwoHeadRanker(in_dim=len(idxs), hidden=224, dropout=0.10).to(DEVICE)
    opt = torch.optim.AdamW(model.parameters(), lr=2e-4, weight_decay=1e-4)
    order = np.arange(len(groups))
    curves = []
    for epoch in range(1, 5):
        np.random.default_rng(SEED + epoch).shuffle(order)
        losses = []
        select_scores, quality_scores, sample_ious = [], [], []
        for gi in order:
            g = groups[int(gi)]
            x = torch.from_numpy(g["x"][:, idxs]).to(DEVICE)
            y = torch.from_numpy(g["y"]).to(DEVICE)
            ranks = torch.from_numpy(g["rank"]).to(DEVICE)
            opt.zero_grad(set_to_none=True)
            select_logit, quality_logit = model(x)
            ...
            best_score = select_logit[best_i]
            select_pair = select_logit.new_tensor(0.0)
            if bool((top_low | above_best).any()):
                select_pair = F.softplus(-(best_score - select_logit[top_low | above_best] - 0.45)).mean()
            select_list = F.kl_div(F.log_softmax(select_logit, dim=0), F.softmax(y / 0.12, dim=0), reduction="batchmean")
            select_bce = F.binary_cross_entropy_with_logits(select_logit, torch.clamp(y, 0.0, 1.0))
            l_select = 0.4 * select_bce + 0.9 * select_pair + 0.8 * select_list

            l_reg = (quality - y).pow(2).mean()
            l_bce05 = F.binary_cross_entropy_with_logits(quality_logit, (y >= 0.5).float())
            l_bce07 = F.binary_cross_entropy_with_logits(quality_logit, (y >= 0.7).float())
            q_pair = quality_logit.new_tensor(0.0)
            hi = y >= 0.7
            lo = y < 0.3
            if bool(hi.any() and lo.any()):
                q_pair = F.softplus(-(quality_logit[hi].mean() - quality_logit[lo].mean() - 0.35))
            # Monotonic pair samples inside the query list.
            yy = y.detach().cpu().numpy()
            if len(yy) > 3:
                order_iou = np.argsort(-yy)
                hi_idx = torch.tensor(order_iou[: min(8, len(order_iou))], device=DEVICE)
                lo_idx = torch.tensor(order_iou[-min(16, len(order_iou)):], device=DEVICE)
                q_pair = q_pair + F.softplus(-(quality_logit[hi_idx].mean() - quality_logit[lo_idx].mean() - 0.25))
            short_w = 1.5 if g["duration_bucket"] == "short" else 1.0
            l_quality = l_reg + 0.35 * l_bce05 + 0.7 * l_bce07 + 0.8 * q_pair
            loss = l_select + 0.9 * l_quality * short_w
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
            opt.step()
            losses.append(float(loss.detach().cpu()))
            if len(sample_ious) < 100000:
                with torch.no_grad():
                    ss, qq = torch.sigmoid(select_logit), torch.sigmoid(quality_logit)
                    select_scores.extend(ss.detach().cpu().tolist())
                    quality_scores.extend(qq.detach().cpu().tolist())
                    sample_ious.extend(y.detach().cpu().tolist())
        sc = c12_5r.corr(select_scores, sample_ious)
        qc = c12_5r.corr(quality_scores, sample_ious)
        curves.append({
            "epoch": epoch,
            "loss": float(np.mean(losses)),
            "select_spearman": sc["spearman"],
            "quality_spearman": qc["spearman"],
            "quality_auc07": c12_5r.auc_score(quality_scores, [x >= 0.7 for x in sample_ious]),
        })
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    path = MODEL_DIR / "c12_5u_two_head_ranker.pt"
    torch.save({"variant": "U1_two_head", "feature_names": [FEATURE_NAMES[i] for i in idxs], "state_dict": model.state_dict(), "curves": curves}, path)
    return {"variant": "U1_two_head", "model": model.eval(), "feature_indices": idxs, "path": str(path), "sha256": sha256_file(path), "curves": curves}
...
def load_rich_ranker(path: Path) -> Tuple[RichRanker, List[int]]:
    ckpt = torch.load(path, map_location=DEVICE)
    names = ckpt["feature_names"]
    idxs = [FEATURE_NAMES.index(n) for n in names]
    model = RichRanker(in_dim=len(idxs), hidden=224 if len(idxs) > 24 else 160).to(DEVICE)
    model.load_state_dict(ckpt["state_dict"])
    return model.eval(), idxs


def score_from_spec(spec: Dict[str, Any], feat: np.ndarray, dur_bucket: str) -> Tuple[np.ndarray, np.ndarray]:
    kind = spec["kind"]
    if kind == "base":
        scores = spec["base_scores"]
        return scores, scores
    if kind == "rich":
        model, idxs = spec["model"], spec["feature_indices"]
        xx = feat[:, idxs]
        chunks = []
        for k in range(0, len(xx), 4096):
            chunks.append(torch.sigmoid(model(torch.from_numpy(xx[k:k + 4096]).to(DEVICE))).detach().cpu().numpy())
        scores = np.concatenate(chunks)
        return scores, scores
    model, idxs = spec["model"], spec["feature_indices"]
    xx = feat[:, idxs]
    selects, qualities = [], []
    for k in range(0, len(xx), 4096):
        s, q = model(torch.from_numpy(xx[k:k + 4096]).to(DEVICE))
        selects.append(torch.sigmoid(s).detach().cpu().numpy())
        qualities.append(torch.sigmoid(q).detach().cpu().numpy())
    select = np.concatenate(selects)
    quality = np.concatenate(qualities)
    if kind == "two_select":
        return select, quality
    if kind == "two_quality":
        return quality, quality
    if kind == "two_select_quality_top100":
        order = np.argsort(-select)
        hybrid = select.copy() * 1e-3
        top = order[:100]
        hybrid[top] = 1.0 + quality[top]
        return hybrid, quality
    if kind == "bucket_quality":
        offset = {"short": 0.03, "medium": 0.0, "long": -0.02}.get(dur_bucket, 0.0)
        return quality + offset, quality + offset
    return select, quality


@torch.no_grad()
def eval_u_variants_joint(
    ...
    store = c12_5.ClipFeatureStore()
    rows_by = {name: [] for name in specs}
    score_by_all = {name: [] for name in specs}
    iou_by_all = {name: [] for name in specs}
    rank_bin_scores = {name: {"top100": ([], []), "top500": ([], [])} for name in specs}
    qtype_by = {name: defaultdict(list) for name in specs}
    dur_by = {name: defaultdict(list) for name in specs}
    hi_low = {name: [] for name in specs}
    lo_hi = {name: [] for name in specs}
    try:
        for st in range(0, len(desc_ids), EVAL_BATCH):
            batch_ids = list(desc_ids[st: st + EVAL_BATCH])
            batch = c12_5.build_batch(corpus, features, store, batch_ids)
            for bi, did in enumerate(batch_ids):
                ...
                qtype = row.get("type", "unknown")
                moment_duration = float(row["ts"][1] - row["ts"][0])
                dur_bucket = duration_bucket_seconds(moment_duration)
                feat = np.asarray([
                    span_feature_row_from_context(ctx, qtype_id(qtype), retr_z, s, e, rank, score, float(meta.get("pq", 0.0)))
                    for rank, (s, e, score, meta) in enumerate(pool)
                ], dtype=np.float32)
                base_scores = np.asarray([p[2] for p in pool], dtype=np.float32)
                ious = []
                for s, e, _score, _meta in pool:
                    ious.append(c12_5.iou_1d(c12_5.idx_to_ts(s, e, float(row["duration"]), t), gt_ts))
                ious_np = np.asarray(ious, dtype=np.float32)
                gt_video_in_top100 = isinstance(first_stage[int(did)].get("rank"), int) and int(first_stage[int(did)]["rank"]) <= 100
                for name, spec in specs.items():
                    spec["base_scores"] = base_scores
                    scores, quality_scores = score_from_spec(spec, feat, dur_bucket)
                    order = np.argsort(-scores)
                    best_generated = float(np.max(ious_np)) if len(ious_np) else 0.0
                    best_idx = int(np.argmax(ious_np)) if len(ious_np) else 0
                    rank_of_best = int(np.where(order == best_idx)[0][0] + 1) if len(order) else None
                    rec = {
                        "desc_id": int(did),
                        "query_type": qtype,
                        "moment_duration": moment_duration,
                        ...
                        "best_generated_iou_top1000": best_generated,
                        "rank_of_best_iou_span": rank_of_best,
                        "top1_start_error": abs(c12_5.idx_to_ts(pool[int(order[0])][0], pool[int(order[0])][1], float(row["duration"]), t)[0] - gt_ts[0]) if len(order) else None,
                        "top1_end_error": abs(c12_5.idx_to_ts(pool[int(order[0])][0], pool[int(order[0])][1], float(row["duration"]), t)[1] - gt_ts[1]) if len(order) else None,
                    }
                    for m in (50, 100, 200, 500):
                        best = float(np.max(ious_np[order[:m]])) if len(order) else 0.0
                        rec[f"best_iou_top{m}"] = best
                        rec[f"cover05_top{m}"] = best >= 0.5
                        rec[f"cover07_top{m}"] = best >= 0.7
                        rec[f"joint05_top{m}"] = gt_video_in_top100 and best >= 0.5
                    ...
                    qtype_by[name][qtype].append(rec)
                    dur_by[name][dur_bucket].append(rec)
                    score_by_all[name].extend(quality_scores.tolist())
                    iou_by_all[name].extend(ious_np.tolist())
                    top100 = order[:100]
                    top500 = order[:500]
                    rank_bin_scores[name]["top100"][0].extend(quality_scores[top100].tolist())
                    rank_bin_scores[name]["top100"][1].extend(ious_np[top100].tolist())
                    rank_bin_scores[name]["top500"][0].extend(quality_scores[top500].tolist())
                    rank_bin_scores[name]["top500"][1].extend(ious_np[top500].tolist())
                    if split == "calib_holdout":
                        high_bad = [int(i) for i in order[:100] if quality_scores[int(i)] >= np.percentile(quality_scores, 90) and ious_np[int(i)] < 0.3]
                        low_good = [int(i) for i in order[-300:] if quality_scores[int(i)] <= np.percentile(quality_scores, 25) and ious_np[int(i)] >= 0.7]
                        for idx in high_bad[:2]:
                            if len(hi_low[name]) < 200:
                                s, e, _, _ = pool[idx]
                                hi_low[name].append({"desc_id": int(did), "query_type": qtype, "duration_bucket": dur_bucket, "score": float(quality_scores[idx]), "iou": float(ious_np[idx]), "rank": int(np.where(order == idx)[0][0] + 1), "span": [int(s), int(e)]})
                        for idx in low_good[:2]:
                            if len(lo_hi[name]) < 200:
                                s, e, _, _ = pool[idx]
                                lo_hi[name].append({"desc_id": int(did), "query_type": qtype, "duration_bucket": dur_bucket, "score": float(quality_scores[idx]), "iou": float(ious_np[idx]), "rank": int(np.where(order == idx)[0][0] + 1), "span": [int(s), int(e)]})
                    spec.pop("base_scores", None)
    finally:
        store.close()

    out = {}
    for name in specs:
        all_scores = score_by_all[name][:300000]
        all_ious = iou_by_all[name][:300000]
        calib = {
            **c12_5r.corr(all_scores, all_ious),
            "auc_iou05": c12_5r.auc_score(all_scores, [x >= 0.5 for x in all_ious]),
            "auc_iou07": c12_5r.auc_score(all_scores, [x >= 0.7 for x in all_ious]),
            "sample_count": min(len(score_by_all[name]), 300000),
            "top100_spearman": c12_5r.corr(rank_bin_scores[name]["top100"][0][:300000], rank_bin_scores[name]["top100"][1][:300000])["spearman"],
            "top500_spearman": c12_5r.corr(rank_bin_scores[name]["top500"][0][:300000], rank_bin_scores[name]["top500"][1][:300000])["spearman"],
        }
        out[name] = {
            "variant": name,
            "split": split,
            ...
            "query_type_breakdown": {k: summarize_rows(v) for k, v in sorted(qtype_by[name].items())},
            "pq_iou_calibration": calib,
            "high_score_low_iou_cases": hi_low[name],
            "low_score_high_iou_cases": lo_hi[name],
        }
    return out
...
def write_u_outputs(results: Dict[str, Any], decision: Dict[str, Any], two_head_info: Dict[str, Any], schema_hash: str) -> None:
    hold = results["calib_holdout"]
    write_json(OUT / "C12_5U_E_RESULTS.json", results)
    write_json(OUT / "C12_5U_E_DURATION_BREAKDOWN.json", {k: hold[k]["duration_breakdown"] for k in hold})
    write_json(OUT / "C12_5U_E_QUERY_TYPE_BREAKDOWN.json", {k: hold[k]["query_type_breakdown"] for k in hold})
    write_json(OUT / "C12_5U_E_CALIBRATION_METRICS.json", {k: {s: results[s][k]["pq_iou_calibration"] for s in ("calib_select", "calib_holdout")} for k in hold})
    write_json(OUT / "C12_5U_E_FAILURE_CASES.json", {k: {"high_score_low_iou": hold[k]["high_score_low_iou_cases"], "low_score_high_iou": hold[k]["low_score_high_iou_cases"]} for k in hold})
    write_json(OUT / "C12_5U_B_TWO_HEAD_RANKER_RESULTS.json", two_head_info)
    write_text(OUT / "C12_5U_B_TWO_HEAD_RANKER_RESULTS.md", f"""# C12-5U-B Two-Head Ranker Results

status = C12_5U_TWO_HEAD_TRAINED

model = {two_head_info['path']}
sha256 = {two_head_info['sha256']}

final training curve = {two_head_info['curves'][-1]}

official_val_used = false
""")
    cal = {k: hold[k]["pq_iou_calibration"] | {
        "top100_IoU@0.7": hold[k]["summary"]["GT_video_oracle_IoU@0.7_top100"],
        "top50_IoU@0.7": hold[k]["summary"]["GT_video_oracle_IoU@0.7_top50"],
    } for k in hold if k.startswith("U")}
    write_json(OUT / "C12_5U_C_CALIBRATION_REPAIR_RESULTS.json", cal)
    lines = ["# C12-5U-C Calibration Repair Results", ""]
    for k, v in cal.items():
        lines += [f"## {k}", f"- Spearman: {v['spearman']}", f"- top100 Spearman: {v['top100_spearman']}", f"- AUC@0.7: {v['auc_iou07']}", f"- IoU@0.7 top100/top50: {v['top100_IoU@0.7']} / {v['top50_IoU@0.7']}", ""]
    write_text(OUT / "C12_5U_C_CALIBRATION_REPAIR_RESULTS.md", "\n".join(lines))
    lines = ["# C12-5U-E Results", ""]
    for k, rec in hold.items():
        s = rec["summary"]
        c = rec["pq_iou_calibration"]
        short = rec["duration_breakdown"].get("short", {})
        lines += [
            ...
            f"- IoU@0.7 top500: {s['GT_video_oracle_IoU@0.7_top500']:.4f}",
            f"- generated M1000 IoU@0.7: {s['generated_M1000_oracle_IoU@0.7']:.4f}",
            f"- best-IoU top100/top50/top10: {s['best_iou_span_top100_rate']} / {s['best_iou_span_top50_rate']} / {s['best_iou_span_top10_rate']}",
            f"- best-IoU mean/median rank: {s['best_iou_span_mean_rank']} / {s['best_iou_span_median_rank']}",
            f"- short IoU@0.7 top100: {short.get('GT_video_oracle_IoU@0.7_top100')}",
            f"- PQ Spearman: {c['spearman']}",
            f"- top100 Spearman: {c['top100_spearman']}",
            f"- AUC@0.7: {c['auc_iou07']}",
            "",
        ]
    write_text(OUT / "C12_5U_E_RESULTS.md", "\n".join(lines))
    write_json(OUT / "C12_5U_DECISION.json", decision)
    write_text(OUT / "C12_5U_DECISION.md", f"""# C12-5U Decision
...
nms_modified = false
""")
    write_json(OUT / "C12_5U_D_FEATURE_SCHEMA_HASH.json", {"stage": "C12-5U-D", "feature_schema_hash": schema_hash, "feature_names": FEATURE_NAMES, "official_val_used": False})
    write_text(OUT / "C12_5U_D_CANDIDATE_VIDEO_SCORE_SEMANTICS.md", f"""# C12-5U-D Candidate-Video Score Semantics
...
official_val_used = false
""")
    t2 = hold["C12_5T_T2_listwise_soft_iou"]
    audit = {
        "stage": "C12-5U-A",
        "status": "C12_5U_SCORE_DECOMPOSITION_COMPLETE",
        "global_spearman": t2["pq_iou_calibration"]["spearman"],
        "top100_spearman": t2["pq_iou_calibration"]["top100_spearman"],
        "top500_spearman": t2["pq_iou_calibration"]["top500_spearman"],
        "duration_spearman": {k: v["pq_iou_calibration"]["spearman"] if "pq_iou_calibration" in v else None for k, v in t2.get("duration_breakdown", {}).items()},
        "answer": "T2 is an effective topK selector but not a usable calibrator; Spearman is negative globally and remains negative inside top100/top500.",
        "high_score_low_iou_case_count": len(t2["high_score_low_iou_cases"]),
        "low_score_high_iou_case_count": len(t2["low_score_high_iou_cases"]),
        "official_val_used": False,
    }
    write_json(OUT / "C12_5U_A_SCORE_DECOMPOSITION_AUDIT.json", audit)
    write_text(OUT / "C12_5U_A_SCORE_DECOMPOSITION_AUDIT.md", f"""# C12-5U-A Score Decomposition Audit

status = {audit['status']}

T2 global Spearman = {audit['global_spearman']}
T2 top100 Spearman = {audit['top100_spearman']}
T2 top500 Spearman = {audit['top500_spearman']}

Conclusion: {audit['answer']}
...
def main_5u() -> None:
    ...
    holdout_first_stage = load_first_stage(corpus, holdout_ids, top_keep=128, cache_name="first_stage_calib_holdout_top128.pkl" if EVAL_LIMIT == 0 else None)
    select_first_stage = load_first_stage(corpus, select_ids, top_keep=128)
    span_model = c12_5r.load_model("teacher_distilled")
    schema = {"feature_names": FEATURE_NAMES, "retriever_score_z_semantics": "candidate_video_first_stage_score_z", "stage": "C12-5U"}
    schema_hash = hashlib.sha256(json.dumps(schema, sort_keys=True).encode("utf-8")).hexdigest()

    groups, dataset_audit = build_promotion_dataset(span_model, corpus, features, train_first_stage, corpus.splits["train_fit"])
    write_json(OUT / "C12_5U_PROMOTION_DATASET_AUDIT.json", dataset_audit)
    two = train_two_head_ranker(groups)

    s2_model, s2_idxs = load_rich_ranker(MODEL_DIR / "c12_5s_S2_span_context_contrast.pt")
    t2_model, t2_idxs = load_rich_ranker(MODEL_DIR / "c12_5t_T2_listwise_soft_iou.pt")
    specs = {
        "C12_5_teacher_distilled_baseline": {"kind": "base"},
        "C12_5S_S2_span_context_contrast": {"kind": "rich", "model": s2_model, "feature_indices": s2_idxs},
        "C12_5T_T2_listwise_soft_iou": {"kind": "rich", "model": t2_model, "feature_indices": t2_idxs},
        "U1_two_head_select": {"kind": "two_select", "model": two["model"], "feature_indices": two["feature_indices"]},
        "U2_two_head_quality": {"kind": "two_quality", "model": two["model"], "feature_indices": two["feature_indices"]},
        "U3_select_then_quality_top100": {"kind": "two_select_quality_top100", "model": two["model"], "feature_indices": two["feature_indices"]},
        "U4_duration_bucket_quality": {"kind": "bucket_quality", "model": two["model"], "feature_indices": two["feature_indices"]},
    }
    results = {
        "stage": "C12-5U-E",
        "calib_select": eval_u_variants_joint(specs, span_model, corpus, features, select_first_stage, select_ids, "calib_select"),
        "calib_holdout": eval_u_variants_joint(specs, span_model, corpus, features, holdout_first_stage, holdout_ids, "calib_holdout"),
        "official_val_used": False,
    }
    selectable = {k: v for k, v in results["calib_select"].items() if k.startswith("U")}
    selected = max(selectable, key=lambda k: (
        selectable[k]["pq_iou_calibration"]["spearman"] > 0.15,
        selectable[k]["summary"]["GT_video_oracle_IoU@0.7_top100"] >= 55.0,
        selectable[k]["pq_iou_calibration"]["auc_iou07"],
        selectable[k]["summary"]["GT_video_oracle_IoU@0.7_top100"],
    ))
    hrec = results["calib_holdout"][selected]
    h = hrec["summary"]
    c = hrec["pq_iou_calibration"]
    short = hrec["duration_breakdown"].get("short", {})
    no_q_collapse = all(v["GT_video_oracle_IoU@0.7_top100"] >= 45.0 for v in hrec["query_type_breakdown"].values())
    allow = (
        h["GT_video_oracle_IoU@0.7_top100"] >= 55.0
        and h["GT_video_oracle_IoU@0.7_top50"] > 38.871
        and short.get("GT_video_oracle_IoU@0.7_top100", 0.0) >= 45.0
        and (c.get("spearman") or 0.0) > 0.15
        and (c.get("auc_iou07") or 0.0) > 0.567
        and h["GT_video_oracle_IoU@0.5_top100"] >= 80.0
        and no_q_collapse
    )
    if allow:
        status = "C12_SPAN_SCORE_CALIBRATED_CONTINUE_TO_C12_6"
    elif h["GT_video_oracle_IoU@0.7_top100"] >= 55.0 and (c.get("spearman") or 0.0) <= 0.0:
        status = "C12_TOPK_PROMOTION_GOOD_CALIBRATION_WEAK_CONTINUE_WITH_CAUTION"
    elif (c.get("spearman") or 0.0) <= 0.0:
        status = "C12_SPAN_SCORE_STILL_UNRELIABLE_NEED_CONQUER_HIDDEN"
    else:
        status = "C12_TOPK_PROMOTION_ONLY_NEED_CROSS_VIDEO_SIMULATION"
    decision = {
        "stage": "C12-5U-F",
        "status": status,
        ...
        "evaluator_modified": False,
        "nms_modified": False,
        "reason": f"Selected on calib_select only. Holdout top100={h['GT_video_oracle_IoU@0.7_top100']:.4f}, top50={h['GT_video_oracle_IoU@0.7_top50']:.4f}, Spearman={c.get('spearman')}, AUC@0.7={c.get('auc_iou07')}.",
    }
    write_u_outputs(results, decision, {k: v for k, v in two.items() if k != "model"}, schema_hash)
    manifest = {
        "stage": "C12-5U",
        "status": status,
        "best_variant": selected,
        ...
        "evaluator_modified": False,
        "nms_modified": False,
        "artifact_hashes": {str(p.relative_to(ROOT)): sha256_file(p) for p in sorted(OUT.glob("C12_5U*")) if p.is_file()},
    }
    write_json(OUT / "C12_5U_MANIFEST.json", manifest)

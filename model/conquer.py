...
class CONQUER(nn.Module):
    ...
    def get_pred_from_raw_query(self, batch, return_intermediates=False):
        ...
        if return_intermediates:
            QAL_feature, qal_aux = self.query_aware_feature_learning_layer(
                QDF_feature, query_feature, video_mask, query_mask, return_attention=True)
        else:
            qal_aux = None
            QAL_feature = self.query_aware_feature_learning_layer(
                QDF_feature, query_feature, video_mask, query_mask)
        ...
        if return_intermediates:
            intermediates = {
                "query_feature": _query_feature,
                "query_feature_repeated": query_feature,
                "query_mask_repeated": query_mask,
                "video_feature_dict": _video_feature_dict,
                "qdf_feature": QDF_feature,
                "moe_weights_dict": moe_weights_dict,
                "qal_feature": QAL_feature,
                "contextual_qal": Contextual_QAL,
                "g_feature": G,
                "video_mask": video_mask,
                "qal_aux": qal_aux,
                "query_batch": query_batch,
                "shared_video_num": shared_video_num,
                "video_len": video_len,
            }
            return video_similarity_score, begin_score_distribution, end_score_distribution, intermediates

...
def run_builder_rebuild(mode: str, seed: int, max_queries: int | None, chunk_size: int, dry_run: bool) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    t0 = time.time()
    ...
    progress_path = C24H_CACHE / f"C24H_REBUILD_PROGRESS_{mode}{'.dry_run' if dry_run else ''}.json"
    rows: List[Dict[str, Any]] = []
    chunks: List[Dict[str, Any]] = []
    splits = ["train_fit", "calib_select", "calib_holdout"]
    try:
        for split in splits:
            ids = selected_ids(corpus, split, mode, max_queries)
            fs = c22r.load_first_stage_cached(split, ids)
            ...
            for did in ids:
                ...
                for rank, (pos, score) in enumerate(ranklist, start=1):
                    if int(pos) < 0 or int(pos) >= len(corpus.train_videos):
                        continue
                    pairs.append((int(did), str(corpus.train_videos[int(pos)]), float(score), int(rank)))
            for st in range(0, len(pairs), max(1, int(chunk_size))):
                block = pairs[st: st + max(1, int(chunk_size))]
                ct0 = time.time()
                    ...
                    for bi, (did, vid, rscore, rrank) in enumerate(block):
                        ...
                        rec = {
                            ...
                            **t2,
                            ...
                            "feature_hash": stable_hash({"feature_cache": "TVR release via C12/C17 ClipFeatureStore", "query_count": len(ids)}),
                        ...
                        rows.append(rec)
                    if bmn_maps:
                        scores = [float(v.get("bmn_final_score", 0.0)) for m in bmn_maps for v in m.values()]
                        if scores:
                            # margin is top1-top2 among BMN span scores within each candidate.
                            pass
                ...
                chunks.append({
                    ...
                    "row_count_after_chunk": len(rows),
    ...
    if len(df):
        for col in ["bmn_final_score", "t2_score", "first_stage_score"]:
            z_cols(df, col, {"bmn_final_score": "bmn_z", "t2_score": "t2_z", "first_stage_score": "first_stage_z"}[col])
        # Compute simple within-video/query margins from rebuilt candidate scores.
        for score_col, margin_col in [("bmn_final_score", "bmn_margin"), ("t2_score", "t2_margin")]:
            val = pd.to_numeric(df[score_col], errors="coerce")
            med = val.groupby([df["split"], df["query_id"]], observed=True).transform("median")
            df[margin_col] = val - med
        df["t2_agreement"] = 1.0 - (pd.to_numeric(df["bmn_z"], errors="coerce") - pd.to_numeric(df["t2_z"], errors="coerce")).abs()
    manifest = {
        ...
        "split_coverage": split_coverage(df) if len(df) else {},
...
def stage_c24h_2(mode: str, seed: int, force: bool = False, dry_run: bool = False, max_queries: int | None = None, chunk_size: int = 16, write_outputs: bool = True) -> Dict[str, Any]:
    df, manifest = run_builder_rebuild(mode, seed, max_queries=max_queries, chunk_size=chunk_size, dry_run=dry_run)
    C24H_CACHE.mkdir(parents=True, exist_ok=True)
    path = c24h_rebuild_path(mode, dry_run=dry_run)
    df.to_parquet(path, index=False)
    manifest.update({"path": str(path), "size_bytes": path.stat().st_size, "sha256": sha256_file(path), "generation_command": f"{PYTHON} run_c24h_full_bmn_t2_trainfit_rebuild.py --stage c24h_2 --mode {mode} --seed {seed} --max_queries {max_queries} --chunk_size {chunk_size}{' --dry_run' if dry_run else ''}"})
    coverage = evidence_coverage(df) if len(df) else {"row_count": 0, "evidence_by_split": {}}
    attempted_splits = sorted(coverage.get("evidence_by_split", {}).keys())
    full_complete = mode == "full" and all(s in attempted_splits for s in ["train_fit", "calib_select", "calib_holdout"]) and all(v.get("bmn_coverage", 0.0) >= 0.95 and v.get("t2_coverage", 0.0) >= 0.95 for v in coverage["evidence_by_split"].values())
    ...
    progress = {
        ...
        "estimated_remaining_note": "full requires all train_fit/calib_select/calib_holdout first-stage top128; current run is bounded by max_queries",
    ...
    runtime = {"runtime_sec": manifest.get("runtime_sec"), "torch_cuda_available": torch.cuda.is_available(), "device": str(c17.DEVICE), "chunks": manifest.get("chunks")}
    rec = {
        "stage": "C24H-2",
        "status": status,
        "mode": mode,
        "seed": seed,
        "dry_run": dry_run,
        "bmn_rebuild_manifest": manifest,
        "t2_rebuild_manifest": manifest,
        "rebuild_progress": progress,
        "chunk_manifest": manifest.get("chunks"),
    ...
    if write_outputs:
        ...
        write_json(OUT2 / "C24H_2_BMN_REBUILD_MANIFEST.json", manifest)
        write_json(OUT2 / "C24H_2_T2_REBUILD_MANIFEST.json", manifest)
        write_json(OUT2 / "C24H_2_REBUILD_PROGRESS.json", progress)
        write_json(OUT2 / "C24H_2_CHUNK_MANIFEST.json", manifest.get("chunks"))
...
def stage_c24h_3(mode: str, seed: int, force: bool = False) -> Dict[str, Any]:
    base = pd.read_parquet(c24f_table_path(mode))
...
def stage_c24h_4(mode: str, seed: int, force: bool = False) -> Dict[str, Any]:
    ...
    if not rebuilt_complete:
        status = "C24H_FULL_EVIDENCE_INCONCLUSIVE" if len(df) else "C24H_FULL_EVIDENCE_TEST_BLOCKED"
    selected = {"name": None, "selected": False, "reason": "C24H rebuild is partial; formula selection is blocked."}
    rec = {"stage": "C24H-4", "status": status, "mode": mode, "seed": seed, "evidence_signal_audit": signal, "front_rank_signal_audit": {"rebuilt_complete": rebuilt_complete, "coverage": coverage}, "complementarity_audit": signal, "integration_results": {"selection_blocked": not rebuilt_complete}, "component_ablation": {"selection_blocked": not rebuilt_complete}, "wrong_video_audit": {c: high_score_fp(df, c) for c in ["bmn_z", "t2_z", "event_z"] if len(df) and c in df}, "selected_integration": selected, "final_vcmr_metrics": {}, "official_val_used": False, "official_prediction_pool_used": False, "pseudo_official_holdout_used_for_selection": False}
...
def stage_c24h_6(mode: str, seed: int, recs: Dict[str, Dict[str, Any]] | None = None) -> Dict[str, Any]:
    ...
    packet = {"decision": decision, "rebuild_actually_attempted": rec["rebuild_actually_attempted"], "bmn_train_fit_coverage": rec["bmn_train_fit_coverage"], "t2_train_fit_coverage": rec["t2_train_fit_coverage"], "commands": rec["rebuild_commands_logs_summary"]}
...
def run_all(args: argparse.Namespace) -> Dict[str, Dict[str, Any]]:
    ...
    r2 = stage_c24h_2(args.mode, args.seed, force=args.force, dry_run=args.dry_run, max_queries=args.max_queries, chunk_size=args.chunk_size)
...
def main() -> None:
    ...
    if args.stage == "c24h_0":
        ...
        rec = stage_c24h_2(args.mode, args.seed, args.force, args.dry_run, args.max_queries, args.chunk_size)

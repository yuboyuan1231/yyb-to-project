...
OUTPUT="$ROOT/results/rlem_c35_qsp/matched_extended_epoch5"
LOG="$ROOT/c35_audit/C35_QSP_MATCHED_EPOCH5_TRAINING.log"
DEVICE="${C35_DEVICE:-cuda:0}"

cd "$ROOT"
if [[ -d "$OUTPUT" ]] && find "$OUTPUT" -mindepth 1 -print -quit | grep -q .; then
  echo "Refusing to overwrite existing epoch-5 output: $OUTPUT" >&2
  exit 2
fi
...
echo "[done] epoch-5 matched training complete; diagnostics/probe not yet run."
